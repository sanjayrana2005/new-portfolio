            ...
            <ol className="relative border-s border-default">
                {
                    timeline && timeline.map((time) => {
                        return (
                            <li key={time._id} className="mb-10 ms-6">
                                <span className="absolute flex items-center justify-center w-6 h-6 bg-brand-softer rounded-full -start-4 ring-4 ring-buffer">
                                ...
                                <h3 className="my-2 text-lg font-bold text-heading">{time.title}</h3>
                                <p className="text-body text-gray-400">{time.description}</p>
                                <time className="bg-neutral-secondary-medium border border-default-medium text-heading text-xs font-medium px-1.5 py-0.5 rounded text-gray-400">{time.timeline.from} - {time.timeline.to ? time.timeline.to : "Present"}</time>
                            </li>
                        )
                    })
                }
